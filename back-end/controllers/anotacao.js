...
controller.novo = async (req, res) => {
    ...
    try {
        await Anotacao.create(req.body)
        // HTTP 201: Created
        res.status(201).end()
    }
    catch (erro) {
...
controller.obterUm = async (req, res)  => {
    try {
        ...
        let obj = await Anotacao.findById(id)
...
controller.atualizar = async (req, res)  => {
    try {
        ...
        let ret = await Anotacao.findByIdAndUpdate(id, req.body)
...
controller.delete = async (req, res)  => {
    try {
        const id = req.body._id
        let ret = await Anotacao.findByIdAndDelete(id, req.body)
        //Se encontrou e achou
        if(ret)res.status(204).end()
        //Não encontrou
        else res.status(404).end()
