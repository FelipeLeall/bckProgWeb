...
controller.listar = async (req, res) => {
    try {
        let dados = await Texto.find() //Traz todos os cursos cadastrados
        res.send(dados)
...
controller.obterUm = async (req, res)  => {
    try {
        ...
        let obj = await Texto.findById(id)
...
controller.atualizar = async (req, res)  => {
    try {
        ...
        let ret = await Texto.findByIdAndUpdate(id, req.body)
...
controller.delete = async (req, res)  => {
    try {
        const id = req.body._id
        let ret = await Texto.findByIdAndDelete(id, req.body)
        //Se encontrou e achou
        if(ret)res.status(204).end()
        //Não encontrou
        else res.status(404).end()
