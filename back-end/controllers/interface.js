...
controller.novo = async (req, res) => {
    ...
    try {
        await Interface.create(req.body)
        // HTTP 201: Created
        res.status(201).end()
    }
    catch (erro) {
...
controller.listar = async (req, res) => {
    try {
        let dados = await Interface.find() //Traz todos os cursos cadastrados
        res.send(dados)
...
controller.obterUm = async (req, res)  => {
    try {
        ...
        let obj = await Interface.findById(id)
...
controller.atualizar = async (req, res)  => {
    try {
        ...
        let ret = await Interface.findByIdAndUpdate(id, req.body)
...
controller.delete = async (req, res)  => {
    try {
        const id = req.body._id
        let ret = await Interface.findByIdAndDelete(id, req.body)
        //Se encontrou e achou
        if(ret)res.status(204).end()
        //Não encontrou
        else res.status(404).end()
